...
<html>
  <head>
    ...
    jQuery(function($){
		$('#userTable').datagrid({
			...
			url:"businessbase/queryList", //数据来源
			sortName: 'id', //排序的列
			sortOrder: 'desc', //倒序
			...
			columns:[[
				...
				{field:'name',title:'商家名称',width:20,sortable:true,
					formatter:function(value,row,index){return row.name;} //需要formatter一下才能显示正确的数据
				},	
				{field:'address',title:'商家地址',width:30,sortable:true,
					formatter:function(value,row,index){return row.address;}
				},
				{field:'contractname',title:'联系人',width:20,sortable:true,
					formatter:function(value,row,index){
						return row.contractname;  //该列的值是deptId，显示的是deptName
					}
				},
				{field:'productcatalogueid',title:'商家产品分类',width:20,sortable:true,
					formatter:function(value,row,index){
						return cataloguecode(row.productcatalogueid); 
					}
				},
				{field:'status',title:'商家状态',width:10,sortable:true,
					formatter:function(value,row,index){
						if(row.status == 0){return "正常"; }
						else{return "失效";}
					}
				},
				 {field:'opt',title:'查看详情',width:30,align:'center',  
					formatter:function(value,row,index){
						 var btn = '<a class="easyui-linkbutton" onclick="viewDetails(\''+row.id+'\')"  href="javascript:void(0)">查看详情</a>'; 
						return btn; 
					}
		        } 
			]],
	...
	function cataloguecode(code){

		code += "";
		var codelength = code.length / 4;
		var start = 0;
		var end = 4;
		var codename  = "";
		for(var i=0; i < codelength; i++){
			if(code.substring(start, end) == "1001"){
		   		codename += "送水 ";
			}else if(code.substring(start, end) == "1002"){
				codename += "洗衣 ";
			}else if(code.substring(start, end)== "1003"){
				codename += "洗车 ";
			}else if(code.substring(start, end) == "1004"){
				codename += "保洁 ";
			}
			start = start+4;
			end = end+4;
  ...
  <body>
    <form id="queryForm" style="margin:10;text-align: center;">
		<table width="100%">
			<tr>
				<td>商家名称：<input name="businessBase.name" style="width: 200;"></td>
				<td>联系人：<input name="userBase.name" style="width: 200" ></td>
