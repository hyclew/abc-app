...
<html>
    ...
    jQuery(function($){
		$('#userTable').datagrid({
			...
			columns:[[
				...
				{field:'user.name',title:'商家名称',width:20,sortable:true,
					formatter:function(value,row,index){return row.user.name;} //需要formatter一下才能显示正确的数据
				},
				{field:'user.address',title:'商家地址',width:30,sortable:true,
					formatter:function(value,row,index){return row.user.age;}
				},
				{field:'user.contractname',title:'联系人',width:20,sortable:true,
					formatter:function(value,row,index){
						return row.deptName;  //该列的值是deptId，显示的是deptName
					}
				},
				{field:'user.productcatalogueid',title:'商家分类',width:20,sortable:true,
					formatter:function(value,row,index){
						return row.deptName;  //该列的值是deptId，显示的是deptName
					}
				},
				{field:'user.phone',title:'商家电话',width:30,sortable:true,
					formatter:function(value,row,index){
						return row.deptName;  //该列的值是deptId，显示的是deptName
					}
				},
				{field:'user.status',title:'商家状态',width:10,sortable:true,
					formatter:function(value,row,index){
						return row.deptName;  //该列的值是deptId，显示的是deptName
					}
				}
  ...
  <body>
    <form id="queryForm" style="margin:10;text-align: center;">
		<table width="100%">
			<tr>
				<td>商家名称：<input name="name" style="width: 200;"></td>
				<td>商家电话：<input name="phone" style="width: 200" ></td>
