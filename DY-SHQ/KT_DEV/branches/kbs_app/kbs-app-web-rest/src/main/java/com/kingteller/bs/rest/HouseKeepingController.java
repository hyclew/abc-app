...
public class HouseKeepingController {
	 ...
	 * @return
	 */
	@RequestMapping(value = "/housekeepinformations/{catalogueid}/{maxId}/{dataCount}", method = RequestMethod.GET)
	public RestResponse waterInformationsList(@PathVariable Long catalogueid, @PathVariable Long maxId, @PathVariable int dataCount){
		logger.info("开始查询家政列表，查询的最大ID是:" + maxId + ",查询条数是:" + dataCount);
		...
		try {
			...
			if(CollectionUtils.isEmpty(products)){
				...
				logger.info("查询到家政信息的数量是:" + products.size());
			...
			logger.error("查询家政列表信息出错，错误信息为:" + e.getLocalizedMessage(), e);
		}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/housekeepdetail/{businessProductId}", method = RequestMethod.GET)
	public RestResponse waterDetailInfo(@PathVariable Long businessProductId){
		logger.info("开始查询家政服务详细信息，查询ID为:" + businessProductId);
		RestResponse response = new RestResponse();
		ResponseHeader header = new ResponseHeader();
		...
		try {
			...
			if(null == product){
				logger.info("家政服务未查询到相关数据");
				header.setErrorCode(ErrorCode.NULL_OR_BLACK);
				header.setMessage("未查询到相关数据");
			}else{
				logger.info("家政服务查询成功，返回信息为:" + product);
				header.setErrorCode(ErrorCode.SUCCESS);
				header.setMessage("查询成功");
			}
		} catch (Exception e) {
			header.setErrorCode(ErrorCode.QUERY_EXCEPTION);
			header.setMessage("查询出现异常");
			logger.error("查询家政详细信息出错，错误信息为:" + e.getLocalizedMessage(), e);
		}
		response.setResponseHeader(header);
